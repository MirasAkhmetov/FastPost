...
object MultipartHelper {
    ...
    fun getFileDataFromPath(name: String, path: String?): MultipartBody.Part? {
    ...
    fun getBitmapDataFromPath(name: String, bitmap: Bitmap?): MultipartBody.Part? {
        if(bitmap == null) return null

        return MultipartHelper.getFileDataFromBitmap(
            name,
            bitmap
        )
    }
